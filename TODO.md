...
- [ ] Implement `SmsReader` — query `content://sms`, filter by sender ID pattern, return `Flow<List<SmsMessage>>`, support date range filtering
...
- [ ] Implement `BankRepositoryImpl` — CRUD for banks and SMS rules
- [ ] Implement `SmsRuleRepositoryImpl` — load active rules by bank, rule priority ordering
